...
class ExperimentCloud {
    ...
    void compute_cloudsize(){
        ...
        accumulator_set<float, stats<tag::min> > min_mr_acc;
        ...
        for(auto&& p : cloud_->points){
            ...
            Eigen::Vector3f diff;
            diff = p.getVector3fMap()-cloud_->points[idx[1]].getVector3fMap();
            min_mr_acc(fabs(diff[0]));
            min_mr_acc(fabs(diff[1]));
            min_mr_acc(fabs(diff[2]));
            acc_x(p.x);
            acc_y(p.y);
            acc_z(p.z);
            acc_range(sq_dist[1]);
        ...
        min_mr_ =  extract::min(min_mr_acc);
        size_   = max_ - min_;
        bbdd_   = size_.norm();
